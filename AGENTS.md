...
- **Refresh tokens rotate** (RFC 9700 §4.14): `POST /api/auth/refresh` returns a **new** refresh token and retires the old one. Rows in `sessions` form a family (`family_id`) per login; rotated rows are kept, not deleted, so a replayed token is detectable — outside `REFRESH_TOKEN_GRACE_PERIOD` it revokes the whole family. `REFRESH_TOKEN_EXPIRY` is a sliding inactivity window, `REFRESH_TOKEN_ABSOLUTE_EXPIRY` a hard ceiling inherited unchanged by every rotation. Never return the same refresh token twice, and never store one unhashed.
...
- **Guild chat** (Discord-style): channels + messages under `/api/guilds/:slug/channels`, realtime via one SSE stream per guild (`GET /api/guilds/:slug/events`). Events go through `internal/platform/realtime.Hub` — Redis pub/sub fan-out when Redis is up, in-process otherwise. SSE handlers must lift the read deadline and set a per-write deadline with `http.ResponseController` (the server's 60s timeouts would cut the stream), and `main.go` closes the hub via `RegisterOnShutdown` so open streams don't stall graceful shutdown. Message history paginates by `?before=<uuidv7 id>` cursor, not offset.
